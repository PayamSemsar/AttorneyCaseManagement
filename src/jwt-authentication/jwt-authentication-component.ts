import {registerAuthenticationStrategy} from '@loopback/authentication';
import {
  Application,
  Binding,
  Component,
  CoreBindings,
  inject,
} from '@loopback/core';
import {
  RefreshTokenServiceBindings,
  RefreshTokenServiceConstants,
  TokenServiceBindings,
  TokenServiceConstants,
  UserServiceBindings,
} from './keys';
import {JWTRefreshService} from './services/jwt-refresh.service';
import {JWTAuthenticationStrategy} from './services/jwt.auth.strategy';
import {JWTService} from './services/jwt.service';
import {MyUserService} from './services/user.service';

export class JWTAuthenticationComponent implements Component {
  bindings: Binding[] = [
    Binding.bind(TokenServiceBindings.TOKEN_SECRET).to(
      TokenServiceConstants.TOKEN_SECRET_VALUE,
    ),
    Binding.bind(TokenServiceBindings.TOKEN_EXPIRES_IN).to(
      TokenServiceConstants.TOKEN_EXPIRES_IN_VALUE,
    ),
    Binding.bind(TokenServiceBindings.TOKEN_SERVICE).toClass(JWTService),




    Binding.bind(RefreshTokenServiceBindings.REFRESH_TOKEN_SECRET).to(
      RefreshTokenServiceConstants.REFRESH_TOKEN_SECRET_VALUE,
    ),
    Binding.bind(RefreshTokenServiceBindings.REFRESH_TOKEN_EXPIRES_IN).to(
      RefreshTokenServiceConstants.REFRESH_TOKEN_EXPIRES_IN_VALUE,
    ),
    Binding.bind(RefreshTokenServiceBindings.REFRESH_TOKEN_SERVICE).toClass(JWTRefreshService),



    Binding.bind(UserServiceBindings.USER_SERVICE).toClass(MyUserService),
  ];
  constructor(@inject(CoreBindings.APPLICATION_INSTANCE) app: Application) {
    registerAuthenticationStrategy(app, JWTAuthenticationStrategy);
  }
}
